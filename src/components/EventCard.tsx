...
const EventCard: React.FC<EventCardProps> = ({ event }) => {
  ...
  const getStatusText = (event: Event) => {
    // upcoming evnt 24h in the future or more 
    if( new Date( event.startTime ).getTime() - new Date().getTime() > 24*60*60*100  ){
      return "UPCOMING"
    }else if(new Date(event.endTime) > new Date() && new Date(event.startTime) > new Date() ){
      return "LIVE" 
